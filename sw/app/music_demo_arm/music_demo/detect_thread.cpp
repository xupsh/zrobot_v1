...
detect_thread::detect_thread(QObject *parent,QString filename) :
    QThread(parent)
{
    file = filename;
...
void  DCT_music(struct segment_music *arry,int length,int num,float *cos_buf)
{
  ...
  for(int i=0;i<num;i++)
  {
    ...
    if(is_find_min&&is_find_max)
    {
      ...
      //printf("segment%d freq_max_n:%d freq_min_n:%d\n",i,freq_max_n,freq_min_n);
...
{
    char *file_in = file.toAscii().data();//输入文件
    ...
    /*对每段进行离散余弦变换，提取基音频率*/
    DCT_music(segment_array,length_segment,num_segment,cos_buf);
